<template>
  <div class="row layout-top-spacing">
    <div class="col-lg-3 col-md-3 col-sm-3 mb-4">
      <input
        ...
        class="form-control"
        required=""
        v-model="store.formreport.search"
        @input="searchData"
        maxlength="30"
      />
    </div>
            ...
            <div class="table-responsive">
              ...
              >
                <thead>
                  <tr class="cours_move">
                    <th>#</th>
                    <th>{{ $t("table_report_learn_fullname_learning") }}</th>
                    <th>{{ $t("table_report_learn_course_code") }}</th>
                    <th>{{ $t("table_report_learn_course_name_lo") }}</th>
                    <th> {{ $t("table_report_learn_course_name_eng") }}</th>
                    <th>{{ $t("table_report_learn_total_learn_lesson") }}</th>
                    <th>{{ $t("table_report_learn_total_lesson") }}</th>
                    <th>{{ $t("table_report_learn_lesson_completion_percentage") }}</th>
                  </tr>
                </thead>

                <tbody>
                  <tr v-for="(item, index) in store.reportcourse">
                    <td>{{ (store.formreport.page * store.formreport.per_page) - (store.formreport.per_page -  index) +  1 }}</td>
                    <td>
                      {{ item.fullname_learning }}
                    </td>
                    <td>
                      {{ item.course_code }}
                    </td>
                    ...
                    <td>
                      {{ item.course_name_eng }}
                    </td>
                    <td>
                      {{ item.total_learn_lesson }}
                    </td>
                    <td>
                      {{ item.total_lesson }}
                    </td>
                    <td>
                      {{ item.lesson_completion_percentage }}
                    </td>
                  </tr>
                </tbody>
              </table>
  ...
  <div  class="dt--bottom-section d-sm-flex justify-content-sm-between text-center">
      ...
      >Showing
      {{
        (store.reportcourse_current_page - 1) * store.formreport.per_page +
        1
      }}
      to
      {{
        Math.min(
          store.reportcourse_current_page * store.formreport.per_page,
          store.reportcourse_total_page
        )
      }}
      of {{ store.reportcourse_total }} entries</span
    >
                </div>
              </div>
              <div class="dt--pagination" v-if="store.reportcourse_total_page > 1">
      <div
      ...
      >
        <ul class="pagination">
          ...
          <li
            ...
            v-for="page in store.reportcourse_total_page"
            :key="page"
          >
            <a
              href="#"
              ...
              tabindex="0"
              class="page-link"
              :class="{ bgc: page === store.formreport.page }"
...
const setCurrentPageclick = async (page) => {
  ...
  await store.setCurrentPageReport(page);
  await store.ReportCourse();
