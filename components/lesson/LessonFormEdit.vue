<template>
        ...
        <div class="row layout-top-spacing">
          <div class="container">
            ...
            />
                  ...
                  <div
                    class="row mb-4 g-3"
                    v-if="store.selectedEditFiles.length > 0"
                  >
 ...
 <span
  ...
  <div class="border p-2 mt-3">
        ...
        <template v-if="store.cs_cover">
          <div class="row">
            <div id="image-container" class="col-md-3 col-sm-4 col-6">
              <div class="image-wrapper">
                <img  :src="coverimage(store.cs_cover)" class="img-fluid" />
  ...
  setup() {
    ...
    const onReadyTh = (editorInstance) => {
      editorInstance.model.document.on("change:data", () => {
        store.setHtmlEditContent(editorInstance.getData());
      });
    };
    ...
    validateAllFields();
    if (formErrors.cs_name_lo == true  && formErrors.cs_name_eng == true && formErrors.cg_id == true && formErrors.cs_description == true) {
      ...
      let uploadimage = await store.UploadfileLessonEdit();   ///upload File 
   let uploadfile = await store.UploadfileLessonEditPdf();   ///upload File รูป
   let update = await store.updateformnewLesson();
      if(update == true){
      ...
      if (event.target.files.length > 0) {
        ...
        for (var i = 0; i < event.target.files.length; i++) {
          ...
          if (file && file.type.startsWith("application/pdf")) {
            store.selectedFiles = [];
            store.selectedEditFiles = [];
            store.selectedFiles.push(event.target.files[i]);
            store.selectedEditFiles.push(event.target.files[i]);
          } else {
...
const router = useRouter();
    return {
      ...
      fileInputRef,
      remove,
      onFileChangeBack,coverimage,locale,removeImage,router
    };
  },
});
</script>