...
import Approvestaff from '@/components/users/Approvestaff.vue'
...
store.formapprove.user_id = auth.user_id;
  ...
  <div v-if="store.PopupIdcard" class="modal">
    <div class="modal-content" id="deleteConformationLabel">
      <div class="modal-body d-flex w-100">
        ...
        <div class="w-100 modal-image" v-if="store.display_user.card">
          <div><img :src="coverimage(store.display_user.card.idcard_front)" alt="" ></div>
          <div><img :src="coverimage(store.display_user.card.idcard_back)" alt="" ></div>
        </div>
        <div class="w-100 modal-image" v-else>
        ...
        <div class="w-100 content-text">
           <p class="">{{ $t("approve_table_view_name") }} : {{ store.display_user.user_firstname }} {{ store.display_user.user_lastname }} </p>
          <p class="">{{ $t("approve_table_view_address") }} : {{ store.display_user.detail.user_address }} {{ store.display_user.detail.location.zipcode_name }} {{ store.display_user.detail.location.province_name }}  {{ store.display_user.detail.country.capital_name_eng }}</p>
          <p class="">{{ $t("approve_table_view_card") }} : {{ store.display_user.detail.identification_number }}</p>
          <p class="">{{ $t("approve_table_view_bday") }} : {{ store.display_user.detail.user_birthday }}</p>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn" data-bs-dismiss="modal" @click="closeModalView">{{ $t("menu_exam_modal_close") }}</button>
      </div>
    </div>
  </div>
</template>
